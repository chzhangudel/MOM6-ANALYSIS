...
class topo:
    ...
    def heaviside1(x, x0):
        """Returns 1 for x < x0, 0 or x >= x0"""
        b = 0*x
        b[x<=x0] = 1
        return b
    def box1(x, x0, x1, y, y0, y1):
        """Returns 1 for x < x0, 0 or x0 <= x <= x1, 1 for x > x1
           Returns 1 for y < y0, 0 or y0 <= y <= y1, 1 for y > y1
        """
        return numpy.maximum.reduce([topo.heaviside1(x, x0),topo.heaviside1(-x, -x1),topo.heaviside1(y, y0),topo.heaviside1(-y, -y1)])
    def cone(x, x0, dx):
    ...
    def add_spoon(self, lon0, lon1, lat0, lat1, dedge):
        lenlon = lon1-lon0
        lenlat = lat1-lat0
        D00 = (self.D0-dedge)/((1-numpy.exp(-0.5*lenlat*6.378E+06 *numpy.pi/(180.0 *4.0E+05)))**2)
        self.z = -( dedge+D00*(numpy.sin(numpy.pi*( self.XC-lon0 )/lenlon)*(1-numpy.exp( (self.YC-lat1)*6.378E+06*numpy.pi/(180*4.0E+05) )) ) ) 
    def add_box(self, lon0, lon1, lat0, lat1):
        self.z = numpy.maximum(self.z, - self.D0 * topo.box1(self.XC, lon0, lon1, self.YC, lat0, lat1) )
